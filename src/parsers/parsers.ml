...
let register_parser ~lang new_parser =
  if List.mem_assoc lang !parsers then
    Util.Printer.print_wrn
      "A parser for extension %S is already registered. It will be hidden !"
      lang;
...
let find_parser ext_opt format =
  ...
  with Not_found ->
    if String.equal ext_opt ".why" then (
      Util.Printer.print_wrn
        "please use the AB-Why3 plugin for file in Why3 format. .why and .mlw \
         extensions are depreciated and used for Why3 files. Continue with \
         native Alt-Ergo parsers!";
...
let get_input_parser fmt =
  ...
  match fmt with
  | Util.Options.Native -> find_parser ".ae" "native"
  | Util.Options.Smtlib2 -> find_parser ".smt2" "smtlib2"
  | Util.Options.Why3 -> find_parser ".why" "why3"
  | Util.Options.Unknown s -> find_parser s s

let get_parser ext_opt =
  if Util.Options.get_infer_input_format () then
    match ext_opt with
...
let parse_input_file file =
  if get_verbose () then
    Util.Printer.print_dbg ~module_name:"Parsers"
      ~function_name:"parse_input_file" "parsing file \"%s\"" file;
  let cin, lb, opened_cin, ext =
    if Filename.check_suffix file ".zip" then
      ...
      else (stdin, Lexing.from_channel stdin, false, ext)
  in
  try
