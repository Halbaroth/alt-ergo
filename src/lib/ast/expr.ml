...
and decl_kind = Dtheory | Daxiom | Dgoal | Dpredicate of t | Dfunction of t
...
type term_view = Term of view | Not_a_term of { is_lit : bool }
...
let compare_lists l1 l2 cmp_elts =
  let res = Util.Util.cmp_lists l1 l2 cmp_elts in
  if res <> 0 then raise (Util.Util.Cmp res)
...
module H = struct
  ...
  let disable_weaks () = Util.Options.get_disable_weaks ()
end
...
and print_verbose fmt t = fprintf fmt "(%a : %a)" print_silent t Ty.print t.ty

and print fmt t =
  if Util.Options.get_debug () then print_verbose fmt t else print_silent fmt t
...
and print_triggers fmt trs =
  List.iter (fun { content = l; _ } -> fprintf fmt "| %a@," print_list l) trs
...
let is_model_label =
  let model = "model:" in
  fun h ->
    try String.equal (String.sub (Util.Hstring.view h) 0 6) model
    with Invalid_argument _ -> false
...
let print_tagged_classes =
  let is_labeled t = not (Util.Hstring.equal (label t) Util.Hstring.empty) in
  fun fmt l ->
    List.iter
      (fun cl ->
...
let fresh_name ty = mk_term (Sy.name (Util.Hstring.fresh_string ())) [] ty
...
let mk_and f1 f2 is_impl id = neg @@ mk_or (neg f1) (neg f2) is_impl id
let mk_imp f1 f2 id = mk_or (neg f1) f2 true id
let mk_xor f1 f2 id = neg (mk_iff f1 f2 id)

let mk_if cond f2 f3 id =
  mk_or (mk_and cond f2 true id) (mk_and (neg cond) f3 true id) false id
  ...
  fun new_q _id ->
    ...
    if is_ground f then f
      ...
      try
        ...
        Util.Printer.print_wrn ~warning:(get_debug_warnings ())
      ...
      with Not_found | Exit ->
        ...
        let vty =
          if new_q.toplevel then Ty.Svty.empty else free_type_vars new_q.main
        in
        let vars =
          Sy.Map.filter
...
let no_occur_check v e = not (Sy.Map.mem v e.vars)
let no_vtys l = List.for_all (fun e -> Ty.Svty.is_empty e.vty) l
...
let mk_distinct ~iff tl =
  match tl with [ a; b ] -> neg (mk_eq ~iff a b) | _ -> neg (mk_nary_eq tl)
...
let no_capture_issue s_t binders =
  ...
  if Sy.Map.is_empty capt_bind then true
    ...
    Util.Printer.print_wrn "captures between@,%aand%a!@,(captured = %a)"
      (Sy.Map.print print) s_t print_binders binders print_binders capt_bind;
    false)
...
let rec apply_subst_aux (s_t, s_ty) t =
  if is_ground t || (Sy.Map.is_empty s_t && Ty.M.is_empty s_ty) then t
    ...
    if Sy.Map.is_empty s_t && Ty.M.is_empty s_ty then t
      ...
      let s = (s_t, s_ty) in
      let xs', same = Util.Lists.apply (apply_subst_aux s) xs in
      ...
      | Sy.Form (Sy.F_Clause b), _ -> (
          match xs' with [ u; v ] -> mk_or u v b 0 | _ -> assert false)
      | Sy.Form Sy.F_Iff, _ -> (
          match xs' with [ u; v ] -> mk_iff u v 0 | _ -> assert false)
      | Sy.Form Sy.F_Xor, _ -> (
          match xs' with [ u; v ] -> mk_xor u v 0 | _ -> assert false)
      | _ -> mk_term f xs' ty'
...
and mk_let_aux ({ let_v; let_e; in_e; _ } as x) =
  try
    ...
    if
      (nb_occ = 1 && (let_e.pure (*1*) || Sy.equal let_v in_e.f))
      || const_term let_e
    then
...
let apply_subst s t =
  if Util.Options.get_timers () then (
    try
      Util.Timers.exec_timer_start Util.Timers.M_Expr Util.Timers.F_apply_subst;
      let res = apply_subst s t in
      Util.Timers.exec_timer_pause Util.Timers.M_Expr Util.Timers.F_apply_subst;
      res
    with e ->
      Util.Timers.exec_timer_pause Util.Timers.M_Expr Util.Timers.F_apply_subst;
      raise e)
...
let resolution_triggers ~is_back { kind; main = f; binders; _ } =
  if Util.Options.get_no_backward () then []
  else
    match kind with
...
let skolemize { main = f; binders; sko_v; sko_vty; _ } =
  ...
  let pp_list fmt l = pp_print_list ~pp_sep:pp_sep_nospace print fmt l in
...
let elim_iff f1 f2 id ~with_conj =
  if with_conj then mk_and (mk_imp f1 f2 id) (mk_imp f2 f1 id) false id
  else
    mk_or (mk_and f1 f2 false id) (mk_and (neg f1) (neg f2) false id) false id
...
module Triggers = struct
  ...
  let vars_of_term bv acc t =
    Sy.Map.fold
      (fun v _ acc -> if Sy.Set.mem v bv then Sy.Set.add v acc else acc)
      t.vars acc
  ...
  *)
  ...
  (* constant terms such as "logic nil : 'a list" are
     allowed in triggers *)
  ...
  let underscore =
    let aux t s =
      ...
      else (apply_subst (sbt, Ty.esubst) t, false)
    in
  ...
  let parties mconf bv vty l escaped_vars =
    let l =
      if mconf.Util.Util.triggers_var then l
  ...
  let simplification =
    let strict_subset bv vty =
      List.exists (fun (_, bv', vty') ->
          Sy.Set.subset bv bv'
          && (not (Sy.Set.equal bv bv'))
          && Svty.subset vty vty'
          || Svty.subset vty vty'
             && (not (Svty.equal vty vty'))
             && Sy.Set.subset bv bv')
    in
  ...
  let multi_triggers menv bv vty trs escaped_vars =
    ...
    let lm = if menv.Util.Util.triggers_var then mt @ mv else mt in
    let m = at_most menv.Util.Util.nb_triggers lm in
    at_most menv.Util.Util.nb_triggers m
  ...
  let mono_triggers menv vterm vtype trs =
    ...
    let base = if menv.Util.Util.triggers_var then trs_nv @ trs_v else trs_nv in
    at_most menv.Util.Util.nb_triggers (List.map (fun (t, _, _) -> [ t ]) base)
  ...
  let potential_triggers =
    let has_bvar bv_lf bv = Sy.Map.exists (fun e _ -> Sy.Set.mem e bv) bv_lf in
    let has_tyvar vty vty_lf = Svty.exists (fun e -> Svty.mem e vty) vty_lf in
    let args_of e lets =
      match e.bind with
    ...
    in
    fun vars e -> aux vars (STRS.empty, Sy.Map.empty) e
...
let debug_compile_match e cases res =
  if get_debug_adt () then (
    Util.Printer.print_dbg ~flushed:false ~module_name:"Expr"
...
let mk_match e cases =
  ...
  let mk_destr =
    match ty with
    | Ty.Tadt _ -> fun hs -> Sy.destruct ~guarded:true (Util.Hstring.view hs)
...
type th_elt = {
  ...
  ax_name : string;
  ax_form : t;
  extends : Util.Util.theories_extensions;
  axiom_kind : Util.Util.axiom_kind;
}
