...
module type ATOM = sig
  ...
  and atom = {
    ...
    lit : Expr.t;
    neg : atom;
    mutable watched : clause Util.Vec.t;
  ...
  val make_clause :
    string -> atom list -> Expr.t -> int -> bool -> premise -> clause
...
(*
  ...
  val ma : var Expr.Map.t ref


  val make_var : Expr.t -> var * bool

  val get_atom : Expr.t -> atom (* get existing atom of a lit *)
  val vrai_atom  : atom
  val faux_atom  : atom
  ...
  val iter_atoms_of_clauses : clause Util.Vec.t -> (atom -> unit) -> unit
...
module Atom : ATOM = struct
  ...
  let dummy_lit = Expr.vrai
  let vraie_form = Expr.vrai
  ...
  and dummy_atom =
    {
      ...
      timp = 0;
      lit = dummy_lit;
      watched = { Util.Vec.dummy = dummy_clause; data = [||]; sz = 0 };
      neg = dummy_atom;
      is_true = false;
      is_guard = false;
      aid = -102;
  ...
  and dummy_clause =
    {
      name = "";
      atoms = { Util.Vec.dummy = dummy_atom; data = [||]; sz = 0 };
      activity = -1.;
      removed = false;
      learnt = false;
      cpremise = [];
  ...
  module Debug = struct
    ...
    let atom fmt a =
      fprintf fmt "%s%d%s [index=%d | lit:%a] vpremise={{%a}}" (sign a)
        (a.var.vid + 1) (value a) a.var.index Expr.print a.lit premise
        a.var.vpremise

    let atoms_vec fmt vec =
      for i = 0 to Util.Vec.size vec - 1 do
        fprintf fmt "%a ; " atom (Util.Vec.get vec i)
      done
  ...
  let normal_form lit =
    (* XXX do better *)
    let is_pos = Expr.is_positive lit in
    ((if is_pos then lit else Expr.neg lit), not is_pos)

  let max_depth a = Expr.depth a
  let literal a = a.lit
  ...
  module HT = Hashtbl.Make (Expr)

  type hcons_env = { tbl : var HT.t; cpt : int ref }

  let make_var hcons lit acc =
      ...
      and pa =
        {
          var;
          lit;
          watched = Util.Vec.make 10 dummy_clause;
          neg = na;
          is_true = false;
          is_guard = false;
          timp = 0;
  ...
  let empty_hcons_env, vrai_atom =
    ...
    let a, _ = add_atom empty_hcons Expr.vrai [] in
    a.is_true <- true;
    a.var.level <- 0;
    a.var.reason <- None;
  ...
  let get_atom hcons lit =
    ...
    with Not_found -> (
        try (HT.find hcons.tbl (Expr.neg lit)).na with Not_found -> assert false)

  let make_clause name ali f sz_ali is_learnt premise =
    let atoms = Util.Vec.from_list ali sz_ali dummy_atom in
    {
      name;
      atoms;
      removed = false;
  ...
  let reason_atoms a =
    ...
    | Some c ->
      ...
      for i = 0 to Util.Vec.size c.atoms - 1 do
        ...
        if eq_atom a b then incr cpt else l := b :: !l
      done;
      if !cpt <> 1 then (
        Util.Printer.print_err "cpt = %d@ a = %a@ c = %a" !cpt pr_atom a
          pr_clause c;
        assert false);
      !l
...
module type FLAT_FORMULA = sig
  ...
  val get_proxy_of :
    t -> (Atom.atom * Atom.atom list * bool) Util.Util.MI.t -> Atom.atom option

  val cnf_abstr :
    hcons_env ->
    t ->
    (Atom.atom * Atom.atom list * bool) Util.Util.MI.t ->
    Atom.var list ->
    Atom.atom
...
module Flat_Formula : FLAT_FORMULA = struct
  ...
  type hcons_env = { tbl : t HT.t; cpt : int ref; atoms : Atom.hcons_env }
  ...
  let mk_lit hcons a acc =
    ...
    let at =
      ...
      else if at.Atom.var.Atom.level = 0 then
        if at.Atom.is_true then Atom.vrai_atom
        else if at.Atom.neg.Atom.is_true then Atom.faux_atom
        else at
      else at
    in
  ...
  let empty_hcons_env, vrai =
    ...
    in
    let vrai = mk_lit empty_hcons Expr.vrai [] |> fst in
    let f_empty_hcons () =
      {
  ...
  let merge_and_check l1 l2 =
    ...
    | h1 :: t1, h2 :: _ ->
      ...
      else if compare h1 h2 < 0 then merge_rec l1 l2 h1
      ...
      let so, nso =
        List.fold_left
          (fun ((so, nso) as acc) e ->
             ...
             | UNIT a
               when (not (get_disable_flat_formulas_simplification ()))
                 && a.Atom.var.Atom.level = 0 ->
               if a.Atom.neg.Atom.is_true then (
                 aaz a;
                 raise Exit);
               (* XXX*)
               if a.Atom.is_true then (
                 aaz a;
                 acc)
               else (so, e :: nso)
             | _ -> (so, e :: nso))
  ...
  let intersect_list l1 l2 =
    let rec inter l1 l2 acc =
      ...
      | f1 :: r1, f2 :: r2 ->
        ...
        else if c > 0 then inter l1 r2 acc
  ...
  let extract_common l =
    ...
    | _ :: _ :: _, _ ->
      if get_debug_sat () then
        Util.Printer.print_dbg ~module_name:"Satml_types"
  ...
  let abstract_lemma hcons abstr (f : Expr.t) tl lem new_vars =
    try fst (abstr f)
    with Not_found -> (
        try fst (snd (List.find (fun (x, _) -> Expr.equal f x) !lem))
        with Not_found ->
          if tl then (
            lem := (f, (vrai, Atom.vrai_atom)) :: !lem;
            vrai)
          else
            let lit = Expr.fresh_name Ty.Tbool in
  ...
  let simplify hcons f abstr new_vars =
    ...
    let rec simp topl ~parent_disj f =
      ...
      | Expr.Literal a ->
        ...
        new_vars := l;
        ff
      | Expr.Lemma _ -> abstract_lemma hcons abstr f topl lem new_vars
      | Expr.Skolem _ -> mk_not (simp false ~parent_disj:false (Expr.neg f))
      | Expr.Unit (f1, f2) -> (
          let x1 = simp topl ~parent_disj:false f1 in
          let x2 = simp topl ~parent_disj:false f2 in
          match (x1.view, x2.view) with
      ...
      | Expr.Iff (f1, f2) ->
        ...
        @@ Expr.elim_iff f1 f2 (Expr.id f) ~with_conj:(not parent_disj)
      | Expr.Xor (f1, f2) ->
        let g =
          Expr.neg @@ Expr.elim_iff f1 f2 (Expr.id f) ~with_conj:parent_disj
        in
        simp topl ~parent_disj g
      | Expr.Let letin ->
        simp false ~parent_disj:false (Expr.elim_let ~recursive:true letin)
    in
  ...
  let mk_new_proxy n =
    let hs = Util.Hstring.make ("PROXY__" ^ string_of_int n) in
    let name = Sy.Name (hs, Sy.Other) in
    Expr.mk_term name [] Ty.Tbool

  let get_proxy_of f proxies_mp =
    try
      let p, _, _ = Util.Util.MI.find f.tag proxies_mp in
      Some p
    with Not_found -> (
        try
          let p, _, _ = Util.Util.MI.find f.neg.tag proxies_mp in
          Some p.Atom.neg
        with Not_found -> None)
    ...
    let rec abstr f new_proxies proxies_mp new_vars =
      ...
      | AND l | OR l -> (
          ...
          | None ->
            ...
            let proxies_mp =
              Util.Util.MI.add f.tag (p, l, is_and) proxies_mp
            in
...
module Proxy_formula = struct
  let get_proxy_of f proxies =
    try Some (Expr.Map.find f proxies) with Not_found -> None
  ...
  let mk_new_proxy n =
    let name = Sy.name @@ "PROXY__" ^ string_of_int n in
    Expr.mk_term name [] Ty.Tbool
  ...
  let rec mk_cnf hcons f ((proxies, inv_proxies, new_vars, cnf) as accu) =
    ...
    | Some p -> (p, accu)
    | None -> (
        let nf = Expr.neg f in
        match get_proxy_of nf proxies with
        (* maybe redundant *)
        | Some p -> (Atom.neg p, accu)
        | None -> (
            let a, new_vars =
              atom_of_lit hcons (mk_new_proxy (Expr.hash f)) false new_vars
            in
            ...
            | Expr.Unit (f1, f2) ->
              ...
              let a2, (proxies, inv_proxies, new_vars, cnf) =
                mk_cnf hcons f2 accu
              in
              let cnf =
                [ na; a1 ] :: [ na; a2 ]
                :: [ a; Atom.neg a1; Atom.neg a2 ]
                :: cnf
              in
            ...
            | Expr.Clause (f1, f2, _) ->
              ...
              in
              (a, (proxies, inv_proxies, new_vars, cnf))
            | Expr.Let _ | Expr.Skolem _ | Expr.Lemma _ | Expr.Literal _
            | Expr.Iff _ | Expr.Xor _ ->
              (a, (proxies, inv_proxies, new_vars, cnf))
            | Expr.Not_a_form -> assert false))
end
