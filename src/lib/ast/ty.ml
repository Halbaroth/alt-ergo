...
type adt_constr = {
  constr : Util.Hstring.t;
  destrs : (Util.Hstring.t * t) list;
}

type type_body = Adt of adt_constr list
  ...
  try
    List.iter
      (fun { constr = s; destrs = t } ->
         if Util.Hstring.equal hs s then (
           res := Some t;
           raise Exit))
      cases;
    raise Not_found
...
let print_generic body_of =
  ...
  let rec print body_of fmt = function
    ...
    | Tvar { v; value = Some (Trecord { args = l; name = n; _ } as t) } ->
      if Hashtbl.mem h v then
        fprintf fmt "%a %s" print_list l (Util.Hstring.view n)
      else (
        Hashtbl.add h v ();
    ...
    | Tvar { value = Some t; _ } ->
      (*fprintf fmt "('a_%d->%a)" v print t *)
      print body_of fmt t
    | Text (l, s) -> fprintf fmt "%a <ext>%s" print_list l (Util.Hstring.view s)
    | Tfarray (t1, t2) ->
      fprintf fmt "(%a,%a) farray" (print body_of) t1 (print body_of) t2
    | Tsum (s, _) -> fprintf fmt "<sum>%s" (Util.Hstring.view s)
    | Trecord { args = lv; name = n; lbs = lbls; _ } ->
      fprintf fmt "%a <record>%s" print_list lv (Util.Hstring.view n);
      if body_of != None then (
        fprintf fmt " = {";
        ...
        List.iter
          (fun (s, t) ->
             fprintf fmt "%s%s : %a"
               (if !first then "" else "; ")
               (Util.Hstring.view s) (print body_of) t;
             first := false)
          lbls;
        fprintf fmt "}")
    | Tadt (n, lv) -> (
        fprintf fmt "%a <adt>%s" print_list lv (Util.Hstring.view n);
        match body_of with
        | None -> ()
        | Some type_body ->
          let cases = match type_body n lv with Adt cases -> cases in
          fprintf fmt " = {";
          let first = ref true in
          List.iter
            (fun { constr = s; destrs = t } ->
               fprintf fmt "%s%s%a"
                 (if !first then "" else " | ")
                 (Util.Hstring.view s) print_adt_tuple t;
               first := false)
            cases;
          fprintf fmt "}")
  and print_adt_tuple fmt = function
    | [] -> ()
    | (d, e) :: l ->
      fprintf fmt " of { %a : %a " Util.Hstring.print d (print None) e;
      List.iter
        (fun (d, e) ->
           fprintf fmt "; %a : %a " Util.Hstring.print d (print None) e)
        l;
      fprintf fmt "}"
...
let rec shorten ty =
  ...
  | Text (l, s) ->
    let l, same = Util.Lists.apply shorten l in
...
and shorten_body _ _ = (() [@ocaml.ppwarning "TODO: should be implemented ?"])
...
let rec compare t1 t2 =
  ...
  | Text (l1, s1), Text (l2, s2) ->
    let c = Util.Hstring.compare s1 s2 in
  ...
  | Tfarray _, _ -> -1
  | _, Tfarray _ -> 1
  | Tsum (s1, _), Tsum (s2, _) -> Util.Hstring.compare s1 s2
  | Tsum _, _ -> -1
  | _, Tsum _ -> 1
...
let rec equal t1 t2 =
  ...
  | Text (l1, s1), Text (l2, s2) -> (
      try Util.Hstring.equal s1 s2 && List.for_all2 equal l1 l2
      with Invalid_argument _ -> false)
  | Tfarray (ta1, ta2), Tfarray (tb1, tb2) -> equal ta1 tb1 && equal ta2 tb2
  | Tsum (s1, _), Tsum (s2, _) -> Util.Hstring.equal s1 s2
  | ( Trecord { args = a1; name = s1; lbs = l1; _ },
      Trecord { args = a2; name = s2; lbs = l2; _ } ) -> (
      try
        Util.Hstring.equal s1 s2 && List.for_all2 equal a1 a2
        && List.for_all2
          (fun (l1, ty1) (l2, ty2) ->
             Util.Hstring.equal l1 l2 && equal ty1 ty2)
          l1 l2
      with Invalid_argument _ -> false)
  | Tint, Tint | Treal, Treal | Tbool, Tbool | Tunit, Tunit -> true
  | Tbitv n1, Tbitv n2 -> n1 = n2
  | Tadt (s1, pars1), Tadt (s2, pars2) -> (
      try Util.Hstring.equal s1 s2 && List.for_all2 equal pars1 pars2
      with Invalid_argument _ -> false (* no need to compare bodies *))
  | _ -> false

(*** matching with a substitution mechanism ***)
module M = Util.Util.MI
...
let rec matching s pat t =
  ...
  | Tvar { v = n; value = None }, _ -> (
      try
        if not (equal (M.find n s) t) then raise (TypeClash (pat, t));
        s
      with Not_found -> M.add n t s)
  | Tvar { value = _; _ }, _ -> raise (Shorten pat)
  | Text (l1, s1), Text (l2, s2) when Util.Hstring.equal s1 s2 ->
    List.fold_left2 matching s l1 l2
  | Tfarray (ta1, ta2), Tfarray (tb1, tb2) ->
    matching (matching s ta1 tb1) ta2 tb2
  | Trecord r1, Trecord r2 when Util.Hstring.equal r1.name r2.name ->
    let s = List.fold_left2 matching s r1.args r2.args in
    List.fold_left2 (fun s (_, p) (_, ty) -> matching s p ty) s r1.lbs r2.lbs
  | Tsum (s1, _), Tsum (s2, _) when Util.Hstring.equal s1 s2 -> s
  | Tint, Tint | Tbool, Tbool | Treal, Treal | Tunit, Tunit -> s
  | Tbitv n, Tbitv m when n = m -> s
  | Tadt (n1, args1), Tadt (n2, args2) when Util.Hstring.equal n1 n2 ->
...
let apply_subst =
  let rec apply_subst s ty =
    match ty with
    | Tvar { v = n; _ } -> ( try M.find n s with Not_found -> ty)
    | Text (l, e) ->
      let l, same = Util.Lists.apply (apply_subst s) l in
    ...
    | Tfarray (t1, t2) ->
      ...
      if t1 == t1' && t2 == t2' then ty else Tfarray (t1', t2')
    | Trecord r ->
      let lbs, same1 = Util.Lists.apply_right (apply_subst s) r.lbs in
      let args, same2 = Util.Lists.apply (apply_subst s) r.args in
      if same1 && same2 then ty
      else Trecord { r with args; name = r.name; lbs }
    | ((Tadt (name, params))
  ...
  | Trecord ({ args; name = n; lbs; _ } as r) ->
    ...
    let lbs, subst =
      List.fold_right
        ...
        lbs ([], subst)
    in
    (Trecord { r with args; name = n; lbs }, subst)
  | Tadt (s, args) ->
    let args, subst = fresh_list args subst in
    (Tadt (s, args), subst)
  | t -> (t, subst)
...
module Decls = struct
  ...
  type decl = { decl : t list * type_body; instances : type_body MTY.t }
  ...
  let body name args =
    try
      ...
      with Not_found ->
          ...
          try
            List.fold_left2
              (fun sbt vty ty ->
                 ...
                 | _ ->
                   Util.Printer.print_err "vty = %a and ty = %a" print vty
                     print ty;
                   assert false)
              M.empty params args
        ...
        body
    with Not_found ->
      Util.Printer.print_err "%a not found" Util.Hstring.print name;
      assert false
end
...
(* smart constructors *)
let tunit = Text ([], Util.Hstring.make "unit")
let text l s = Text (l, Util.Hstring.make s)

let fresh_empty_text =
  let cpt = ref (-1) in
  fun () ->
    incr cpt;
    text [] ("'_c" ^ string_of_int !cpt)

let tsum s lc = Tsum (Util.Hstring.make s, List.map Util.Hstring.make lc)

let t_adt ?(body = None) s ty_vars =
  let hs = Util.Hstring.make s in
  ...
  (match body with
   ...
   | Some ([ _ ] as cases) ->
     if get_debug_adt () then
       Util.Printer.print_dbg ~module_name:"Ty"
         "should be registered as a record";
     let cases =
       List.map
         (fun (s, l) ->
            let l = List.map (fun (d, e) -> (Util.Hstring.make d, e)) l in
            { constr = Util.Hstring.make s; destrs = l })
         cases
     in
...
let trecord ?(record_constr = "{") lv n lbs =
  let lbs = List.map (fun (l, ty) -> (Util.Hstring.make l, ty)) lbs in
  ...
  in
  let record_constr = Util.Hstring.make record_constr in
  Trecord { record_constr; args = lv; name = Util.Hstring.make n; lbs }
...
let rec hash t =
  ...
  | Text (l, s) ->
    abs
      (List.fold_left
         (fun acc x -> (acc * 19) + hash x)
         (Util.Hstring.hash s) l)
  | Tfarray (t1, t2) -> (19 * hash t1) + (23 * hash t2)
  | Trecord { args; name = s; lbs; _ } ->
    let h =
      List.fold_left
        (fun h ty -> (27 * h) + hash ty)
        (Util.Hstring.hash s) args
    in
    let h =
      List.fold_left
        (fun h (lb, ty) -> (23 * h) + (19 * Util.Hstring.hash lb) + hash ty)
        (abs h) lbs
    in
    abs h
  | Tsum (s, _) -> abs (Util.Hstring.hash s) (*we do not hash constructors*)
  | Tadt (s, args) ->
    let h =
      List.fold_left
        (fun h ty -> (31 * h) + hash ty)
        (Util.Hstring.hash s) args
    in
    abs h
  | _ -> Hashtbl.hash t
...
let rec unify t1 t2 =
  ...
  | Tsum (s1, _), Tsum (s2, _) when Util.Hstring.equal s1 s2 -> ()
  | Tint, Tint | Tbool, Tbool | Treal, Treal | Tunit, Tunit -> ()
  | Tbitv n, Tbitv m when m = n -> ()
  | Tadt (n1, p1), Tadt (n2, p2) when Util.Hstring.equal n1 n2 ->
...
let instantiate lvar lty ty =
  let s =
    List.fold_left2
      (fun s x t ->
         match x with Tvar { v = n; _ } -> M.add n t s | _ -> assert false)
      M.empty lvar lty
  in
  apply_subst s ty
...
let rec monomorphize ty =
  ...
  | Tadt (name, params) -> Tadt (name, List.map monomorphize params)
