...
(* TODO: Create a module in Util for this. *)
...
let set_sigprof () =
  let tm =
    let v = Util.Options.get_profiling_period () in
...
let query () = incr state.queries
let instances l = state.instances := !(state.instances) + List.length l

let instantiation ilvl =
  incr state.instantiation_rounds;
  incr state.instantiation_lvl;
  if not (!(state.instantiation_lvl) = ilvl) then (
    Util.Printer.print_err "state.instantiation_lvl = %d et ilvl = %d"
      !(state.instantiation_lvl) ilvl;
    assert false)

let bool_conflict () = incr state.b_conflicts
let theory_conflict () = incr state.th_conflicts
...
let empty_inst_info loc =
  {
    ...
    confl = 0;
    decided = 0;
    all_insts = Expr.Set.empty;
    consumed = Expr.Set.empty;
    all = Expr.Set.empty;
    produced = Expr.Set.empty;
    _new = Expr.Set.empty;
  }
...
let new_instance_of axiom inst loc kept =
  ...
  let ii =
    if kept then
      { ii with kept = ii.kept + 1; all_insts = Expr.Set.add inst ii.all_insts }
...
let decision d origin =
  ...
  if not (!(state.decision_lvl) = d) then (
    Util.Printer.print_err "state.decision_lvl = %d et d = %d"
...
let register_produced_terms axiom loc consumed all produced _new =
  ...
  let ii =
    {
      ii with
      consumed = Expr.Set.union ii.consumed consumed;
      all = Expr.Set.union ii.all all;
      produced = Expr.Set.union ii.produced produced;
      _new = Expr.Set.union ii._new _new;
    }
  in
  state.instances_map := MS.add axiom ii !(state.instances_map)
...
type mode = Stats | Timers | CallTree | FunctionsTimers | Instances
...
let columns =
  [
    ...
    ( "Case splits",
      ...
      fun _ gtime _ sz ->
        let avg =
          int_of_float (float_of_int (Util.Steps.cs_steps ()) /. gtime)
        in
        sprintf "%s~%s"
          (string_resize (sprintf "%d" (Util.Steps.cs_steps ())) (sz - 7))
          (string_resize (sprintf "%d/s" avg) 6) );
    ( "Mod.",
      ...
      None,
      fun _ _ timers sz ->
        let kd, _msg, _ = Util.Timers.current_timer timers in
        string_resize (Util.Timers.string_of_ty_module kd) sz );
    ( "Module Id",
      "Each call to a module is tagged with a fresh Id",
      10,
      None,
      fun _ _ timers sz ->
        let _kd, _msg, id = Util.Timers.current_timer timers in
        int_resize id sz );
    (*-----------------------------------------------------------------*)
    ( "ilvl",
    ...
    ( "#i rnds",
      ...
      8,
      Some true,
      fun _ _ _ sz -> int_resize !(state.instantiation_rounds) sz );
    ( "#insts",
    ...
    ( "B-E",
      ...
      6,
      Some true,
      fun _ _ _ sz -> int_resize !(state.b_elim) sz );
    ( "T-R",
    ...
    ( "T-E",
      ...
      6,
      Some true,
      fun _ _ _ sz -> int_resize !(state.t_elim) sz );
    ( "B-!",
    ...
    ( "SAT",
      ...
      fun _ gtime timers sz ->
        let curr = Util.Timers.get_sum timers Util.Timers.M_Sat in
        sprintf "%s~%s"
    ...
    ( "Matching",
      ...
      Some false,
      fun _ gtime timers sz ->
        let curr = Util.Timers.get_sum timers Util.Timers.M_Match in
        sprintf "%s~%s"
          (float_resize curr (sz - 5))
          (string_resize (percent gtime curr) 4) );
    ( "CC",
      ...
      Some false,
      fun _ gtime timers sz ->
        let curr = Util.Timers.get_sum timers Util.Timers.M_CC in
        sprintf "%s~%s"
          (float_resize curr (sz - 5))
          (string_resize (percent gtime curr) 4) );
    ( "Arith",
      ...
      Some false,
      fun _ gtime timers sz ->
        let curr = Util.Timers.get_sum timers Util.Timers.M_Arith in
        sprintf "%s~%s"
          (float_resize curr (sz - 5))
          (string_resize (percent gtime curr) 4) );
    ( "Arrays",
      ...
      Some false,
      fun _ gtime timers sz ->
        let curr = Util.Timers.get_sum timers Util.Timers.M_Arrays in
        sprintf "%s~%s"
          (float_resize curr (sz - 5))
          (string_resize (percent gtime curr) 4) );
    ( "Sum",
      "Time spent in Sum module(s)",
      16,
      Some false,
      fun _ gtime timers sz ->
        let curr = Util.Timers.get_sum timers Util.Timers.M_Sum in
        sprintf "%s~%s"
          (float_resize curr (sz - 5))
          (string_resize (percent gtime curr) 4) );
    ( "Records",
      "Time spent in Records module(s)",
      16,
      Some false,
      fun _ gtime timers sz ->
        let curr = Util.Timers.get_sum timers Util.Timers.M_Records in
        sprintf "%s~%s"
          (float_resize curr (sz - 5))
          (string_resize (percent gtime curr) 4) );
    ( "AC",
      ...
      Some false,
      fun _ gtime timers sz ->
        let curr = Util.Timers.get_sum timers Util.Timers.M_AC in
        sprintf "%s~%s"
          (float_resize curr (sz - 5))
          (string_resize (percent gtime curr) 4) );
    ( "Total",
      ...
      Some false,
      fun _ _ timers sz ->
        let tsat = Util.Timers.get_sum timers Util.Timers.M_Sat in
        let tmatch = Util.Timers.get_sum timers Util.Timers.M_Match in
        let tcc = Util.Timers.get_sum timers Util.Timers.M_CC in
        let tarith = Util.Timers.get_sum timers Util.Timers.M_Arith in
        let tarrays = Util.Timers.get_sum timers Util.Timers.M_Arrays in
        let tsum = Util.Timers.get_sum timers Util.Timers.M_Sum in
        let trecs = Util.Timers.get_sum timers Util.Timers.M_Records in
        let tac = Util.Timers.get_sum timers Util.Timers.M_AC in
        let total =
          tsat +. tmatch +. tcc +. tarith +. tarrays +. tsum +. trecs +. tac
        in
        float_resize total sz );
  ]

let print_initial_info fmt =
  if !initial_info then (
    initial_info := false;
    let max =
      List.fold_left
...
let print_header header fmt =
  ...
  in
  if header || !nb_prints >= max_nb_prints then (
    nb_prints := 0;
...
let print_instances_generation forced _steps fmt _timers =
  if (not forced) && !(state.instances_map_printed) then
    ...
    (*if forced then
      ...
      let terms = ref Expr.Set.empty in
      List.iter
      (fun (name, i, _) ->
      Expr.Set.iter
      (fun t ->
      ...
      terms := Expr.Set.union !terms i.consumed;
      Expr.Set.iter
      (fun t ->
      fprintf fmt "\"%s\" -> \"%d\";@." name (T.hash t)
      )i._new;
      terms := Expr.Set.union !terms i._new;
    ...
    (*
      ...
      List.iter
      (fun (s2, i2, _) ->
      if Expr.Set.is_empty (Expr.Set.inter i1.produced i2.consumed) then ()
...
let print_call_tree _forced _steps fmt timers =
  ...
  fprintf fmt "(%s, %s, %s)@."
    (string_resize (Util.Timers.string_of_ty_module m) 5)
    (string_resize (Util.Timers.string_of_ty_function f) 10)
    (int_resize id 7)
...
let line_of_module arr fmt f =
  fprintf fmt "%s " (string_resize (Util.Timers.string_of_ty_function f) 13);
  let cpt = ref 0. in
  List.iter
    (fun m ->
...
let line_of_sum_module fmt timers =
  ...
  List.iter
    (fun m -> fprintf fmt "| %a  " float_print (Util.Timers.get_sum timers m))
    Util.Timers.all_modules;
  fprintf fmt "| GTimer %a |@." float_print (Util.Options.Time.value ())
...
let timers_table forced fmt timers =
  ...
  fprintf fmt "|@.";
  let arr_timers = Util.Timers.get_timers_array timers in
  List.iter (line_of_module arr_timers fmt) Util.Timers.all_functions;
