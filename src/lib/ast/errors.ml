...
type error =
  ...
  | Lexical_error of Util.Loc.t * string
  | Syntax_error of Util.Loc.t * string
  | Typing_error of Util.Loc.t * typing_error
...
let warning_as_error () =
  if Util.Options.get_warning_as_error () then error Warning_as_error
...
let report_run_error fmt = function
  ...
  | Steps_limit i -> fprintf fmt "Steps limit reached: %d" i
