...
let print_dbg ?(flushed = true) ?(header = Options.get_output_with_headers ())
  ...
  if header then (
    let fname =
      if String.equal function_name "" then "" else sprintf "[%s]" function_name
    in
    let mname =
      if String.equal module_name "" then "" else sprintf "[%s]" module_name
    in
    (* we force a newline to split the print at every print with header *)
    force_new_line fmt;
    if Options.get_output_with_colors () then
      fprintf fmt "@{<fg_blue>@{<bold>[Debug]%s%s@}@}@,@[<v 0>" mname fname
    else fprintf fmt "[Debug]%s%s@,@[<v 0>" mname fname);
  if flushed || Options.get_output_with_forced_flush () then
    kfprintf flush fmt s
  else fprintf fmt s
...
let pp_list_no_space f fmt l = pp_print_list ~pp_sep:pp_sep_nospace f fmt l

(******** Status printers *********)
let status_time t = match t with None -> "" | Some t -> sprintf " (%2.4f)" t

let status_steps s =
  match s with None -> "" | Some s -> sprintf " (%d steps)" s

let status_goal g = match g with None -> "" | Some g -> sprintf " (goal %s)" g
...
let print_status_value fmt (v, color) =
  ...
  else fprintf fmt "%s" v

let print_status ?(validity_mode = true) ?(formatter = Options.get_fmt_std ())
  ...
  fprintf native_output_fmt "%s%a%a%s%s%s@." comment_if_smt2 print_status_loc
    loc print_status_value (validity_status, color) (status_time time)
    (status_steps steps) (status_goal goal);
...
let print_status_inconsistent ?(validity_mode = true) loc time steps goal =
  print_status ~validity_mode ~formatter:(Options.get_fmt_dbg ())
...
let print_status_timeout ?(validity_mode = true) loc time steps goal =
  print_status ~validity_mode
    ("Timeout", "timeout", "fg_orange")
    loc time steps goal

let print_status_preprocess ?(validity_mode = true) time steps =
  print_status ~validity_mode ~formatter:(Options.get_fmt_dbg ())
    ("Preprocessing", "", "fg_magenta")
    None time steps None
