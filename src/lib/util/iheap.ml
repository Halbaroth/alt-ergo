...
let init size =
  { heap    =  Vec.init ~size ~dummy;
    indices =  Vec.init ~size ~dummy }
...
let grow_by_double { indices; heap } ~limit =
  Vec.grow_by_double indices ~dummy ~limit;
  Vec.grow_by_double heap ~dummy ~limit
