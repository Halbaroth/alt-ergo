...
let name ?(kind = Other) s = Name (Util.Hstring.make s, kind)
let var s = Var s
let int i = Int (Util.Hstring.make i)
let real r = Real (Util.Hstring.make r)
let constr s = Op (Constr (Util.Hstring.make s))
let destruct ~guarded s = Op (Destruct (Util.Hstring.make s, guarded))
...
let is_ac x = match x with Name (_, Ac) -> true | _ -> false
...
let compare_kinds k1 k2 =
  Util.Util.compare_algebraic k1 k2 (function _, (Ac | Other) -> assert false)
...
let hash x =
  ...
  | Name (n, Ac) -> (19 * Util.Hstring.hash n) + 5
  | Name (n, Other) -> (19 * Util.Hstring.hash n) + 6
  | Int n | Real n -> (19 * Util.Hstring.hash n) + 7
...
let string_of_bound_kind x =
  ...
  | ValBnd v -> Util.Numbers.Q.to_string v

let string_of_bound b =
  let kd = string_of_bound_kind b.kind in
  if b.is_lower then Format.sprintf "%s %s" (if b.is_open then "]" else "[") kd
...
let string_of_lit lit =
  ...
  | L_neg_built LE -> ">"
  | L_neg_built LT -> ">="
  | L_built (IsConstr h) -> sprintf "? %s" (Util.Hstring.view h)
  | L_neg_built (IsConstr h) -> sprintf "?not? %s" (Util.Hstring.view h)
...
let to_string ?(show_vars = true) x =
  match x with
  | Name (n, _) -> Util.Hstring.view n
  | Var v when show_vars -> Format.sprintf "'%s'" (Var.to_string v)
  | Var v -> Var.to_string v
  | Int n -> Util.Hstring.view n
  | Real n -> Util.Hstring.view n
  | Bitv s -> "[|" ^ s ^ "|]"
  | Op Plus -> "+"
  | Op Minus -> "-"
  | Op Mult -> "*"
