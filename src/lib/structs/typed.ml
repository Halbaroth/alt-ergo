...
type oplogic = OPand | OPor | OPxor | OPimp | OPnot | OPiff | OPif
...
type 'a tterm = { tt_ty : Ty.t; tt_desc : 'a tt_desc }
...
and 'a tt_desc =
  ...
  | TTdot of 'a atterm * Util.Hstring.t
  | TTrecord of (Util.Hstring.t * 'a atterm) list
  | TTlet of (Sy.t * 'a atterm) list * 'a atterm
  | TTnamed of Util.Hstring.t * 'a atterm
...
and 'a tform =
  ...
  | TFnamed of Util.Hstring.t * 'a atform
  | TFmatch of 'a atterm * (pattern * 'a atform) list

and 'a tlet_kind = TletTerm of 'a atterm | TletForm of 'a atform
...
type tlogic_type = TPredicate of Ty.t list | TFunction of Ty.t list * Ty.t
...
and 'a tdecl =
  ...
  | TTheory of
      Util.Loc.t
      * string
      * Util.Util.theories_extensions
      * ('a tdecl, 'a) annoted list
...
let print_binder fmt (s, t) = fprintf fmt "%a :%a" Sy.print s Ty.print t
...
let rec print_term fmt t =
  ...
  | TTconst Tvoid -> fprintf fmt "void"
  | TTconst (Tint n) -> fprintf fmt "%s" n
  | TTconst (Treal n) -> fprintf fmt "%s" (Num.string_of_num n)
  | TTconst (Tbitv s) -> fprintf fmt "%s" s
...
and print_atom fmt a =
  ...
  | TAeq [ t1; t2 ] -> fprintf fmt "%a = %a" print_term t1 print_term t2
  | TAneq [ t1; t2 ] -> fprintf fmt "%a <> %a" print_term t1 print_term t2
