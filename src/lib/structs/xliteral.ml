...
module Make (X : OrderedType) : S with type elt = X.t = struct
  type elt = X.t
  type atom = { value : elt atom_view; uid : int }
  ...
  let print fmt a =
    let lbl = Util.Hstring.view (label a) in
  ...
  let equal_builtins n1 n2 =
    match (n1, n2) with
    | LT, LT | LE, LE -> true
    | IsConstr h1, IsConstr h2 -> Util.Hstring.equal h1 h2
    | _ -> false
  ...
  let make_aux av is_neg =
    ...
    let at = H.make av in
    if is_neg then
      { at; neg = is_neg; tpos = (2 * at.uid) + 1; tneg = 2 * at.uid }
