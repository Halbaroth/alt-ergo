...
struct
  ...
  let output_used_context g_name dep =
    if not (Util.Options.get_js_mode ()) then (
  ...
  let unused_context name context =
    match context with None -> false | Some s -> not (Util.Util.SS.mem name s)
  ...
  let print_status status steps =
    ...
    in
    let validity_mode =
      match Util.Options.get_output_format () with
      | Smtlib2 -> false
  ...
  let init_with_replay_used acc f =
    ...
    let acc =
      ref (match acc with None -> Util.Util.SS.empty | Some ss -> ss)
    in
    try
      while true do
        acc := Util.Util.SS.add (input_line cin) !acc
      done;
      assert false
    with End_of_file -> Some !acc

  let init_used_context ~goal_name =
    if Util.Options.get_replay_used_context () then
      let uc_f =
        sprintf "%s.%s.used" (Util.Options.get_used_context_file ()) goal_name
      in
  ...
  let init_all_used_context () =
    if Util.Options.get_replay_all_used_context () then
  ...
  let choose_used_context all_ctxt ~goal_name =
    if Util.Options.get_replay_all_used_context () then all_ctxt
