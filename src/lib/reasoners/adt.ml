...
(* TODO: can this function be replace with Structs.Ty.assoc_destrs ?? *)
let constr_of_destr ty dest =
  if get_debug_adt () then
    Util.Printer.print_dbg ~module_name:"Adt" ~function_name:"constr_of_destr"
      "ty = %a" Structs.Ty.print ty;
  match ty with
  | Structs.Ty.Tadt (s, params) -> (
      let bdy = Structs.Ty.type_body s params in
      match bdy with
      | Structs.Ty.Adt cases -> (
          try
            List.find
...
module Shostak (X : ALIEN) = struct
  ...
  let is_mine_symb sy ty =
    ...
    &&
    match (sy, ty) with
    | Structs.Sy.Op (Structs.Sy.Constr _), Structs.Ty.Tadt _ -> true
    | Structs.Sy.Op (Structs.Sy.Destruct (_, guarded)), _ -> not guarded
    | _ -> false
  ...
  let equal s1 s2 =
    ...
    | Constr c1, Constr c2 -> (
        ...
        && Structs.Ty.equal c1.c_ty c2.c_ty
        &&
        try
          List.iter2
            (fun (hs1, v1) (hs2, v2) ->
    ...
    | Select d1, Select d2 ->
        ...
        && Structs.Ty.equal d1.d_ty d2.d_ty
  ...
  let make t =
    ...
    match (f, xs, ty) with
    | Structs.Sy.Op (Structs.Sy.Constr hs), _, Structs.Ty.Tadt (name, params) ->
        let cases =
          match Structs.Ty.type_body name params with
          | Structs.Ty.Adt cases -> cases
        in
        let case_hs =
          try Structs.Ty.assoc_destrs hs cases with Not_found -> assert false
        in
        let c_args =
          try
            List.rev
    ...
    | Structs.Sy.Op (Structs.Sy.Destruct (hs, guarded)), [ e ], _ ->
        if not guarded then
  ...
  let hash x =
    ...
    | Constr { c_name; c_ty; c_args } ->
        List.fold_left
          ...
          (Util.Hstring.hash c_name + (7 * Structs.Ty.hash c_ty))
          c_args
    | Select { d_name; d_ty; d_arg } ->
        ((Util.Hstring.hash d_name + (11 * Structs.Ty.hash d_ty)) * 11)
        + X.hash d_arg
  ...
  (*
    not (get_disable_adts ()) &&
    match sb with
    | Structs.Sy.Op (Structs.Sy.Constr _) -> true
    | Structs.Sy.Op Structs.Sy.Destruct (_, guarded) -> not guarded
    | _ -> false
*)
  ...
  let compare s1 s2 =
    ...
    | Constr c1, Constr c2 -> (
        ...
        if c <> 0 then c
          ...
          let c = Structs.Ty.compare c1.c_ty c2.c_ty in
          if c <> 0 then c
          else
            try
              List.iter2
    ...
    | Select d1, Select d2 ->
        ...
        if c <> 0 then c
          ...
          let c = Structs.Ty.compare d1.d_ty d2.d_ty in
        ...
        | Select ({ d_name; d_arg; _ } as s) ->
            ...
            let cons =
              Structs.Expr.mk_term
                (Structs.Sy.constr (Util.Hstring.view constr))
