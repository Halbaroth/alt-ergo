...
struct
  ...
  module Debug = struct
    let solve_aux r1 r2 =
      if get_debug_arith () then
        Util.Printer.print_dbg ~module_name:"Arith" ~function_name:"solve_aux"
    ...
    let solve_one r1 r2 sbs =
      ...
      in
      if get_debug_arith () then
        Util.Printer.print_dbg ~module_name:"Arith" ~function_name:"solve_one"
  ...
  let is_mine_symb sy _ty =
    ...
    | Int _ | Real _ -> true
    | Op
        ( Plus | Minus | Mult | Div | Modulo | Float | Fixed | Abs_int
  ...
  let mod_sym a b =
    ...
    let m =
      ...
      else if Q.compare m b <= 0 then m
      else assert false
    in
  ...
  let is_null p =
    if Q.sign (snd (P.separate_constant p)) <> 0 then raise Util.Util.Unsolvable;
    []
  ...
  let make t =
    if get_timers () then (
      try
        Util.Timers.exec_timer_start Util.Timers.M_Arith Util.Timers.F_make;
        let res = make t in
        Util.Timers.exec_timer_pause Util.Timers.M_Arith Util.Timers.F_make;
        res
      with e ->
        Util.Timers.exec_timer_pause Util.Timers.M_Arith Util.Timers.F_make;
        raise e)
  ...
  let solve r1 r2 pb =
    if get_timers () then (
      try
        Util.Timers.exec_timer_start Util.Timers.M_Arith Util.Timers.F_solve;
        let res = solve r1 r2 pb in
        Util.Timers.exec_timer_pause Util.Timers.M_Arith Util.Timers.F_solve;
        res
      with e ->
        Util.Timers.exec_timer_pause Util.Timers.M_Arith Util.Timers.F_solve;
        raise e)
