...
module Shostak (X : ALIEN) = struct
  ...
  module Debug = struct
    ...
    let print fmt = function
      | Cons (hs, _) -> fprintf fmt "%s" (Util.Hstring.view hs)
      | Alien x -> fprintf fmt "%a" X.print x

    let solve_bis a b =
      if get_debug_sum () then
        print_dbg ~module_name:"Enum" ~function_name:"solve"
    ...
    let solve_bis_unsolvable () =
      if get_debug_sum () then print_dbg "the equation is unsolvable@]"
  end
  (*BISECT-IGNORE-END*)
  ...
  let solve a b =
    ...
    | Cons (c1, _), Cons (c2, _) when Util.Hstring.equal c1 c2 -> []
    | Cons (_, _), Cons (_, _) -> raise Util.Util.Unsolvable
    | Cons _, Alien r2 -> [ (r2, a) ]
    | Alien r1, Cons _ -> [ (r1, b) ]
    | Alien _, Alien _ -> if X.str_cmp a b > 0 then [ (a, b) ] else [ (b, a) ]
  ...
  let solve r1 r2 pb =
    if Util.Options.get_timers () then (
      try
        Util.Timers.exec_timer_start Util.Timers.M_Sum Util.Timers.F_solve;
        let res = solve r1 r2 pb in
        Util.Timers.exec_timer_pause Util.Timers.M_Sum Util.Timers.F_solve;
        res
      with e ->
        Util.Timers.exec_timer_pause Util.Timers.M_Sum Util.Timers.F_solve;
        raise e)
  ...
  let choose_adequate_model _ r l =
    let l =
      List.filter
        (fun (_, r) -> match embed r with Cons _ -> true | _ -> false)
        l
    in
    let r =
      match l with
