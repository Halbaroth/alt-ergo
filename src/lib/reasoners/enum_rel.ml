...
type 'a abstract = 'a Enum.abstract =
  | Cons of Util.Hstring.t * Structs.Ty.t
  | Alien of 'a
...
let add_diseq hss sm1 sm2 dep env eqs =
  ...
  | Alien r, Cons (h, ty) | Cons (h, ty), Alien r ->
      let enum, ex =
        try MX.find r env.mx with Not_found -> (hss, Structs.Ex.empty)
      in
  ...
  | Alien r1, Alien r2 ->
      let enum1, ex1 =
        try MX.find r1 env.mx with Not_found -> (hss, Structs.Ex.empty)
      in
      let enum2, ex2 =
        try MX.find r2 env.mx with Not_found -> (hss, Structs.Ex.empty)
      in
...
let add_aux env r =
  ...
  | Alien r, Some hss ->
      ...
      else { env with mx = MX.add r (hss, Structs.Ex.empty) env.mx }
  | _ -> env
