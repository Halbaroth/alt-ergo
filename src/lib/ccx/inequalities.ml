...
module Container : Container_SIG = struct
  module Make (P : Polynome.T with type r = Shostak.Combine.r) :
    ...
    module Debug = struct
      ...
      let list_of_ineqs fmt = List.iter (fprintf fmt "%a  " print_inequation)
      ...
      let rec fourier acc mp =
        ...
        if MINEQS.is_empty mp then acc
          ...
          try
            ...
            let mp', nb_inqs =
              if
                Q.compare size_res (get_fm_cross_limit ()) >= 0
                && Q.sign (get_fm_cross_limit ()) >= 0
              then
