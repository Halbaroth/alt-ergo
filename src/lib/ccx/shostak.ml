...
module rec CX : sig
  ...
  module HC = Util.Hcons.Make (View)
  ...
  *)
  ...
  module Debug = struct
    ...
    let print_sbt msg sbs =
      ...
      let print fmt (p, v) =
        incr c;
        fprintf fmt "<%d) %a |-> %a@ " !c print p print v
      in
    ...
    let debug_abstraction_result oa ob a b acc =
      ...
      let print fmt (p, v) =
        incr c;
        fprintf fmt "(%d) %a |-> %a@ " !c CX.print p CX.print v
      in
      if get_debug_combine () then
  ...
  let make_idemp a b sbs =
    ...
    (*
      ...
      assert (not (Util.Options.get_enable_assertions()) ||
