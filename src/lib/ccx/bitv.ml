...
type solver_simple_term_aux = S_Cte of bool | S_Var of tvar
...
module Shostak (X : ALIEN) = struct
    ...
    *)
    ...
    let print_sliced_sys l =
      ...
      if Util.Options.get_debug_bitv () then
        Util.Printer.print_dbg ~module_name:"Bitv" ~function_name:"slicing" "%a"
          (pp_list_no_space print) l

    let print_c_solve_res l =
      let print fmt (a, b) = fprintf fmt " %a == %a@ " print a print_S_ast b in
      if Util.Options.get_debug_bitv () then
        Util.Printer.print_dbg ~module_name:"Bitv" ~function_name:"c_solve"
          "(map)c_solve :@ %a" (pp_list_no_space print) l

    let print_partition_res l =
      let print fmt (t, cte_l) =
        fprintf fmt " %a%a@ " print t
          (fun fmt -> List.iter (fun l' -> fprintf fmt " == %a" print_S_ast l'))
          cte_l
      in
      if Util.Options.get_debug_bitv () then
        Util.Printer.print_dbg ~module_name:"Bitv" ~function_name:"partition"
    ...
    let print_final_solution l =
      ...
      in
      if Util.Options.get_debug_bitv () then
        Util.Printer.print_dbg ~module_name:"Bitv" ~function_name:"solution"
          "%a" (pp_list_no_space print) l
  end
  (*BISECT-IGNORE-END*)
  ...
  module Solver = struct
    ...
    let fresh_bitv genre size =
      if size <= 0 then [] else [ { bv = S_Var (fresh_var genre); sz = size } ]
    ...
    let cte_vs_ext bol xt s_xt i j =
      ...
      let var = { bv = Other xt; sz = s_xt } in
      (var, a2 @ cte @ a1)
    ...
    let other_vs_ext st xt s_xt i j =
      ...
      let a1 = fresh_bitv A i in
      let a2 = fresh_bitv A (s_xt - 1 - j) in
      let extr = { bv = Other xt; sz = s_xt } in
      [ (st, c); (extr, a2 @ c @ a1) ]
    ...
    let apply_subs subs sys =
      let rec f_aux = function
        ...
        | v :: r -> (
            try
              let v1, v2 = List.assoc v subs in
              v1 :: v2 :: f_aux r
            with _ -> v :: f_aux r)
      in
    ...
    let init_sets vals =
      ...
      let f_aux = List.map2 (fun ac_e e -> ST_Set.add e ac_e) in
      List.fold_left f_aux acc tl
    ...
    let build_solution unif_slic sets =
      let get_rep var =
        fst (List.find (fun (_, set) -> ST_Set.mem var set) sets)
      in
    ...
    let solve u v =
      if u = v then raise Valid
        ...
        if varsU = [] && varsV = [] then raise Util.Util.Unsolvable
        else
  ...
  let hash_simple_term_aux = function
    ...
    | Ext (x, a, b, c) -> hash_xterm x + (19 * (a + b + c))

  let hash l =
    List.fold_left
      (fun acc { bv = r; sz } -> acc + (19 * (sz + hash_simple_term_aux r)))
      19 l
  ...
  let solve_bis u t =
    if Util.Options.get_debug_bitv () then
      Util.Printer.print_dbg ~module_name:"Bitv" ~function_name:"solve_bis"
    ...
    | Some u, Some t -> (
        try
          List.map (fun (p, v) -> (var_or_term p, is_mine v)) (Solver.solve u t)
        with Solver.Valid -> [])
  ...
  let subst x subs biv =
    if Util.Options.get_debug_bitv () then
      Util.Printer.print_dbg ~module_name:"Bitv" ~function_name:"subst"
...
*)
  ...
  let choose_adequate_model _ _ = assert false
end
