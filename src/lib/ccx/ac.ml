...
module Make (X : Intf.X.Sig) = struct
  ...
  *)
  ...
  let subst p v ({ Intf.Solvable_theory.h; l; _ } as tm) =
    ...
    Util.Timers.exec_timer_pause Util.Timers.M_AC Util.Timers.F_subst;
    t

  let add h arg arg_l =
    Util.Timers.exec_timer_start Util.Timers.M_AC Util.Timers.F_add;
    let r = compact (flatten h arg arg_l) in
    Util.Timers.exec_timer_pause Util.Timers.M_AC Util.Timers.F_add;
    r
