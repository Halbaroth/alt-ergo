...
module Simplex (C : Coef_Type) = struct
  ...
  module Normalizer = struct
    ...
    let normalize_poly p sbt zsbt =
      for i = 0 to Vec.size sbt - 1 do
        subst_in_p p (Vec.get sbt i)
      done;
      for i = 0 to Vec.size zsbt - 1 do
        z_subst_in_p p (Vec.get zsbt i)
      done
        ...
        List.fold_left
          (fun acc ((_, p) as line) ->
            ...
            if array_is_null p.a2 then
              ...
              if c = 0 then acc
              else if c < 0 then raise Inconsistent
  ...
  module Core_Simplex = struct
    ...
    type i_result = I_unsat of system | I_unbound of system | I_max of system
    ...
    let loops distr order co_cst =
      if C2.compare co_cst !last_cost = 0 then (
        incr cpt;
    ...
    let delete_ghost ghost ghost_p ctx distr order =
      ...
      try
        for i = 0 to !len - 1 do
          if C.compare ghost_p.a2.(i) C.zero <> 0 then (
            ch_vr := i;
            raise Exit)
        done;
    ...
    let infos_of distr q { c2; _ } ctx =
      ...
      let inf i s =
        if s > 0 then
          try acc := (s, List.assoc i acc0) :: !acc with Not_found -> ()
      in
