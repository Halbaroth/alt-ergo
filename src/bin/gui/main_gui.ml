...
module TH = (val if Options.get_no_theory () then
...
let save_dialog cancel envs () =
  if List.exists (fun env -> Stdlib.( <> ) env.actions env.saved_actions) envs
  then
    if List.exists (fun env -> not (Gui_session.safe_session env.actions)) envs
    then
      GToolbox.message_box ~title:"Unsafe session"
      ...
      GToolbox.question_box ~title:"Save session" ~buttons:[ cancel; "Save" ]
        ~default:2
        ~icon:(GMisc.image ~stock:`SAVE ~icon_size:`DIALOG ())
        "Would you like to save the current session ?"
      = 2
    then save_session envs
...
let show_about () =
  let v = "Alt-Ergo" in
  let aw =
    GWindow.about_dialog ~name:v ~position:`CENTER
      ~authors:
        [
          "Sylvain Conchon";
          "Évelyne Contejean";
      ...
      ~copyright:
        "CNRS - INRIA - Université Paris Sud (2006-2013)\nOCamlPro (2013-2018)"
      ~license:"CeCILL-C" ~version:Version._version
      ~website:"http://alt-ergo.lri.fr\nhttp://alt-ergo.ocamlpro.com" ~title:v
      ()
  in
...
let pop_error ?(error = false) ~message () =
  ...
  in
  let bbox =
    GPack.button_box `HORIZONTAL ~border_width:5
      ~layout:`END (*~child_height:20 ~child_width:85*)
  ...
  ignore
    (GMisc.image
       ~stock:(if error then `DIALOG_ERROR else `DIALOG_WARNING)
       ~icon_size:`DIALOG ~packing:hbox#pack ());
  ignore (GMisc.label ~text:message ~xalign:0. ~xpad:10 ~packing:hbox#add ());
  ignore (button_ok#connect#clicked ~callback:pop_w#destroy);
  pop_w#show ()

let pop_model sat_env () =
  let pop_w =
    GWindow.dialog ~title:"Model" ~destroy_with_parent:true ~position:`CENTER
  ...
  let t =
    {
      ...
      pr_sat =
        GRange.progress_bar
          ~packing:(table#attach ~left:2 ~top:0 ~expand:`X ~shrink:`BOTH)
          ();
      pr_match =
        GRange.progress_bar
          ~packing:(table#attach ~left:2 ~top:1 ~expand:`X ~shrink:`BOTH)
          ();
      pr_cc =
        GRange.progress_bar
          ~packing:(table#attach ~left:2 ~top:2 ~expand:`X ~shrink:`BOTH)
          ();
      pr_arith =
        GRange.progress_bar
          ~packing:(table#attach ~left:2 ~top:3 ~expand:`X ~shrink:`BOTH)
          ();
      pr_arrays =
        GRange.progress_bar
          ~packing:(table#attach ~left:2 ~top:4 ~expand:`X ~shrink:`BOTH)
          ();
      pr_sum =
        GRange.progress_bar
          ~packing:(table#attach ~left:2 ~top:5 ~expand:`X ~shrink:`BOTH)
          ();
      pr_records =
        GRange.progress_bar
          ~packing:(table#attach ~left:2 ~top:6 ~expand:`X ~shrink:`BOTH)
          ();
      pr_ac =
        GRange.progress_bar
          ~packing:(table#attach ~left:2 ~top:7 ~expand:`X ~shrink:`BOTH)
          ();
    }
  in
...
let refresh_timers t () =
  ...
  let total =
    tsat +. tmatch +. tcc +. tarith +. tarrays +. tsum +. trecords +. tac
  in
  ...
  Hashtbl.iter
    (fun id (r, n, name, limit) ->
       ...
       if upd_info then (
         ...
         let slimit = if !limit >= 0 then string_of_int !limit else "∞" in
...
let update_status image label buttonclean env s steps =
  ...
  | FE.Unsat (d, dep) ->
    ...
    if not satmode then Printer.print_std "%a" Loc.report d.st_loc;
    if satmode then Printer.print_std "@{<C.F_Red>unsat@}"
    ...
    ignore
      (buttonclean#connect#clicked ~callback:(fun () -> prune_unused env))
  | FE.Inconsistent d ->
    if not satmode then
      Printer.print_err "%a@ Inconsistent assumption" Loc.report d.st_loc
    else Printer.print_std "unsat";
    image#set_stock `EXECUTE;
    label#set_text "  Inconsistent assumption"
  | FE.Unknown (d, t) ->
    if not satmode then
      Printer.print_std "%a@ I don't know." Loc.report d.st_loc
    else Printer.print_std "unknown";
    image#set_stock `NO;
    label#set_text
      (sprintf "  I don't know (%2.2f s)" (Options.Time.value ()));
    if get_model () then pop_model t ()
  | FE.Sat (d, t) ->
    if not satmode then Printer.print_std "%a" Loc.report d.st_loc;
    if satmode then Printer.print_std "unknown (sat)"
    else Printer.print_std "I don't know";
    image#set_stock `NO;
    label#set_text
      (sprintf "  I don't know (sat) (%2.2f s)" (Options.Time.value ()));
    if get_model () then pop_model t ()
  | FE.Timeout _ -> assert false (* should not happen in GUI ? *)
  | FE.Preprocess -> assert false (* should not happen in GUI ! *)
...
let update_aborted image label buttonstop buttonrun timers_model = function
  | Abort_thread ->
    ...
    if get_debug () then Printer.print_dbg "alt-ergo thread terminated";
  ...
  | Alt_ergo_lib_util.Util.Timeout ->
    ...
    Timers.update timers_model.timers;
    if get_debug () then
      Printer.print_dbg "alt-ergo thread terminated (timeout)";
    image#set_stock `CUT;
...
let wrapper_update_status image label buttonclean env s steps =
  GtkThread.sync
    (fun () -> update_status image label buttonclean env s steps)
    ()

let wrapper_update_aborted image label buttonstop buttonrun timers_model e =
  GtkThread.async
    (fun () -> update_aborted image label buttonstop buttonrun timers_model e)
    ()
...
let wrapper_refresh_instances inst_model =
  GtkThread.async (fun () -> ignore (refresh_instances inst_model ()))

let wrapper_refresh_timers timers_model =
  GtkThread.async (fun () -> ignore (refresh_timers timers_model ()))

let interrupt = ref None

let vt_signal =
  match Sys.os_type with "Win32" -> Sys.sigterm | _ -> Sys.sigvtalrm
...
let run_replay env used_context =
  ...
  if get_debug () then
    Printer.print_dbg "AST : @ -----@ %a" print_typed_decl_list ast;
...
let run buttonrun buttonstop buttonclean inst_model timers_model image label
    thread env used_context () =
  Profiling.init ();

  (* Install the signal handler: *)
  let old_action_ref = ref Sys.Signal_ignore in
  let old_action =
    Sys.signal vt_signal (Sys.Signal_handle (force_interrupt old_action_ref))
  in
    ...
    Some
      (Thread.create
         (fun () ->
            (try
               (* Thread.yield (); *)
               if get_debug () then Printer.print_dbg "Starting alt-ergo thread";
               Options.Time.start ();
               Options.Time.set_timeout ~is_gui:true (Options.get_timelimit ());
               ...
               Options.Time.unset_timeout ~is_gui:true
             with e ->
               wrapper_update_aborted image label buttonstop buttonrun
                 timers_model e);
            if get_debug () then
              Printer.print_dbg "Send done signal to waiting thread";
...
let remove_context env () =
  List.iter
    (fun (td, _) ->
       match td.c with
       | APredicate_def (_, _, _, _) -> toggle_prune env td
       | AAxiom (_, s, _, _)
         when String.length s = 0
           || (Stdlib.( <> ) s.[0] '_' && Stdlib.( <> ) s.[0] '@') ->
         toggle_prune env td
       | _ -> ())
    env.ast
...
let create_error_view error_model buffer sv ~packing () =
  ...
  let col =
    GTree.view_column ~title:""
      ~renderer:(renderer, [ ("stock_id", error_model.rcol_icon) ])
      ()
  in
  ...
  let col =
    GTree.view_column ~title:"Line"
      ~renderer:(renderer, [ ("text", error_model.rcol_line) ])
      ()
  in
  ignore (view#append_column col);
  ...
  let col =
    GTree.view_column ~title:"Description"
      ~renderer:(renderer, [ ("text", error_model.rcol_desc) ])
      ()
  in
  ignore (view#append_column col);
  col#set_resizable true;
...
let goto_lemma (view : GTree.view) inst_model buffer
  ...
  try
    ...
    env.last_tag#set_properties [ `BACKGROUND_SET false; `UNDERLINE_SET false ];
...
let set_color_inst inst_model renderer (istore : GTree.model) row =
  ...
  let limit = !limit in
  if nb_inst = limit then renderer#set_properties [ `FOREGROUND "blue" ]
  ...
  else renderer#set_properties [ `FOREGROUND_SET false ];
  Thread.yield ()
...
let create_inst_view inst_model env buffer sv ~packing () =
  ...
  let col =
    GTree.view_column ~title:""
      ~renderer:(renderer, [ ("stock_id", inst_model.icol_icon) ])
      ()
  in
  ignore (view#append_column col);
  ...
  let col =
    GTree.view_column ~title:"#"
      ~renderer:(renderer, [ ("text", inst_model.icol_number) ])
      ()
  in
  ignore (view#append_column col);
  col#set_cell_data_func renderer (set_color_inst inst_model renderer);
  col#set_resizable true;
  col#set_sort_column_id inst_model.icol_number.GTree.index;

  let renderer = GTree.cell_renderer_text [ `EDITABLE true ] in
  ignore
    (renderer#connect#edited ~callback:(fun _path s ->
         ...
         List.iter
           (fun path ->
              ...
              if limit >= 0 then (
                ...
                Gui_session.save env.actions
                  (Gui_session.UnlimitLemma (id, name))))
           view#selection#get_selected_rows));
  let col =
    GTree.view_column ~title:"limit"
      ~renderer:(renderer, [ ("text", inst_model.icol_limit) ])
      ()
  in
  ignore (view#append_column col);
  col#set_resizable true;
  ...
  let renderer = GTree.cell_renderer_text [] in
  let col =
    GTree.view_column ~title:"Lemma"
      ~renderer:(renderer, [ ("text", inst_model.icol_desc) ])
      ()
  in
  ignore (view#append_column col);
  col#set_cell_data_func renderer (set_color_inst inst_model renderer);
...
let next_begins i buf found_all_tag =
  let iter = ref i in
  while
    !iter#compare buf#end_iter < 0
    && not (!iter#begins_tag (Some found_all_tag))
  do
...
let prev_ends i buf found_all_tag =
  let iter = ref i in
  while
    !iter#compare buf#start_iter > 0
    && not (!iter#ends_tag (Some found_all_tag))
  do
...
let search_next ?(backward = false) (sv : GSourceView3.source_view)
  ...
  try
    ...
    ignore (sv#scroll_to_iter ~use_align:true ~yalign:0.1 i1#backward_line);
...
let search_all entry (_sv : GSourceView3.source_view) (buf : sbuffer) found_tag
    found_all_tag () =
  buf#remove_tag found_tag ~start:buf#start_iter ~stop:buf#end_iter;
...
let start_gui all_used_context =
  ...
  let main_vbox =
    GPack.vbox ~homogeneous:false ~border_width:0 ~packing:w#add ()
  in

  let menubar = GMenu.menu_bar ~packing:main_vbox#pack () in

  let notebook =
    GPack.notebook ~border_width:0 ~tab_pos:`BOTTOM ~show_border:false
      ~enable_popup:true ~scrollable:true ~packing:main_vbox#add ()
  in

  let note_search = Hashtbl.create 7 in

  let session_cin =
    try Some (open_in_bin (get_session_file ())) with Sys_error _ -> None
  in
    ...
    List.fold_left
      (fun acc (l, goal_name) ->
         ...
         let buf1 =
           ...
           | Some language ->
             GSourceView3.source_buffer ~language ~highlight_syntax:true
               ~highlight_matching_brackets:true ()
           | None -> GSourceView3.source_buffer ()
         in
         ...
         if get_debug () then
           Printer.print_dbg ~flushed:false "Computing dependencies...";
         let dep = make_dep annoted_ast in
         if get_debug () then Printer.print_dbg ~header:false "Done";
         ...
         let append g =
           nb_page := notebook#append_page ~tab_label:label#coerce g
         in

         let eventBox = GBin.event_box ~border_width:0 ~packing:append () in

         let vbox =
           GPack.vbox ~homogeneous:false ~border_width:0 ~packing:eventBox#add ()
         in
         ...
         let toolbar =
           GButton.toolbar (*~tooltips:true*)
             ~packing:toolbox#add ~style:`BOTH ()
         in
         toolbar#set_icon_size `DIALOG;

         let hb = GPack.paned `HORIZONTAL ~border_width:3 ~packing:rbox#add () in

         let vb1 =
           GPack.paned `VERTICAL ~border_width:3
             ~packing:(hb#pack1 ~shrink:true ~resize:true)
             ()
         in
         let vb2 =
           GPack.paned `VERTICAL ~border_width:3
             ~packing:(hb#pack2 ~shrink:true ~resize:true)
             ()
         in

         let fr1 =
           GBin.frame ~shadow_type:`ETCHED_OUT
             ~width:(60 * window_width / 100)
             ~height:(50 * window_height / 100)
             ~packing:(vb1#pack1 ~shrink:true ~resize:true)
             ()
         in
         ...
         let fr2 =
           GBin.frame ~shadow_type:`ETCHED_OUT
             ...
             ~packing:(vb2#pack1 ~shrink:true ~resize:true)
             ()
         in

         let fr3 =
           GBin.frame ~shadow_type:`ETCHED_OUT ~show:false
             ~height:(5 * window_height / 100)
             ~packing:(vb1#pack2 ~shrink:true ~resize:true)
             ()
         in

         let binfo =
           GPack.vbox ~border_width:0
             ~packing:(vb2#pack2 ~shrink:true ~resize:true)
             ()
         in

         let fr4 = GBin.frame ~shadow_type:`ETCHED_OUT ~packing:binfo#add () in

         let fr5 = GBin.frame ~shadow_type:`NONE ~packing:binfo#pack () in

         let table_timers =
           GPack.table ~columns:3 ~rows:8 ~row_spacings:1 ~col_spacings:8
             ~border_width:4 ~packing:fr5#add ()
         in

         let st = GMisc.statusbar ~border_width:0 ~packing:vbox#pack () in
         ...
         let env =
           create_env buf1 tv1 buf2 tv2 error_model inst_model st_ctx annoted_ast
             dep actions resulting_ids
         in
         connect env;

         let remove_ctx_button =
           GButton.toggle_tool_button ~label:" Remove context" ~stock:`CUT
         ...
         let run_button =
           GButton.tool_button ~label:" Run Alt-Ergo" ~stock:`EXECUTE ()
         in
         ...
         let result_image =
           GMisc.image ~icon_size:`LARGE_TOOLBAR ~stock:`DIALOG_QUESTION
             ~packing:result_box#add ()
         in
         let result_label = GMisc.label ~text:" " ~packing:result_box#add () in
         ...
         in
         ignore
           (GMisc.image ~icon_size:`LARGE_TOOLBAR ~stock:`FIND
              ~packing:search_box#add ());
         ...
         ignore
           (search_entry#connect#changed
              ~callback:
                (search_all search_entry tv1 buf1 found_tag found_all_tag));

         ignore
           (search_entry#event#connect#key_press ~callback:(fun k ->
                if GdkEvent.Key.keyval k = GdkKeysyms._Return then (
                  search_next tv1 buf1 found_tag found_all_tag ();
                  true)
                else false));

         ignore
           (search_forw_button#connect#clicked
              ~callback:(search_next tv1 buf1 found_tag found_all_tag));
         ignore
           (search_back_button#connect#clicked
              ~callback:
                (search_next ~backward:true tv1 buf1 found_tag found_all_tag));
         ...
         in
         ignore
           (create_error_view error_model env.buffer tv1 ~packing:sw3#add ());
         ...
         ignore
           (create_inst_view inst_model env env.buffer tv1 ~packing:sw4#add ());
         ...
         ignore (stop_button#connect#clicked ~callback:(kill_thread thread));

         ignore (eventBox#event#connect#key_press ~callback:(set_ctrl env true));

         ignore
           (eventBox#event#connect#key_release ~callback:(set_ctrl env false));
  ...
  (match session_cin with Some c -> close_in c | None -> ());
  ...
  let not_implemented _ = Printer.print_err "Not implemented" in
  ...
  let options_entries =
    let set_complete_model b =
      if b then set_model MComplete else set_model MNone
    in
    let set_all_models b = if b then set_model MAll else set_model MNone in
    let set_model b = if b then set_model MDefault else set_model MNone in
    let set_greedy b =
      if b then set_instantiation_heuristic IGreedy
      else set_instantiation_heuristic INormal
    in
    [
  ...
  let help_entries = [ `I ("About", show_about) ] in
  ...
  let key_press ev =
    ...
    let mods_ev =
      List.filter (fun m -> not (List.mem m mod_mask)) (GdkEvent.Key.state ev)
    in
    match mods_ev with
    | [ `CONTROL ] -> (
        match key_ev with
        | k when k = GdkKeysyms._q ->
          quit envs ();
          true
        | k when k = GdkKeysyms._s ->
          save_dialog "Cancel" envs ();
          true
        | k when k = GdkKeysyms._f ->
          focus_search ();
          true
        | k when k = GdkKeysyms._r ->
          launch_run ();
          true
...
let start_replay session_cin all_used_context =
  ...
  List.iter
    (fun (l, goal_name) ->
       ...
       (* cradingue *)
       let env =
         create_replay_env buf1 error_model inst_model annoted_ast actions
           resulting_ids
       in
