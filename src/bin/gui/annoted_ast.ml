...
let make_indent nb = String.make (min max_indent (nb * indent_size)) ' '
...
let set_font ?(family = font#family) ?(size = font#size) ?(ratio = 1.) () =
  ...
  font#modify ~family ~size:new_sz ();
...
type buffer_pending = { tags_ranges : (sbuffer * int * int) list HTag.t }

let pending = { tags_ranges = HTag.create 2001 }
...
let append_mark (buffer : sbuffer) id =
  ignore
    (buffer#create_source_mark ~category:(sprintf "trigger_%d" id)
       buffer#end_iter)
...
let create_env buf1 tv1 (buf2 : sbuffer) tv2 errors insts st_ctx ast dep actions
    resulting_ids =
...
and print_tatom fmt a =
  ...
  | TAdistinct tl -> fprintf fmt "distinct(%a)" (print_tterm_list ", ") tl
...
and print_rwt fmt { rwt_vars = rv; rwt_left = rl; rwt_right = rr } =
  fprintf fmt "forall %a. %a = %a" print_var_list rv print_tterm rl print_tterm
    rr
...
and print_quant_form fmt { qf_bvars = bv; qf_triggers = trs; qf_form = tf; _ } =
  fprintf fmt "%a [%a]. %a" print_var_list bv print_triggers trs print_tform tf
...
and make_dep_aform d ex dep = function
  ...
  | AFop (_, afl) -> List.fold_left (make_dep_aaform d ex) dep afl
...
let rec of_tterm (buffer : sbuffer) t =
  { at_desc = of_tt_desc buffer t.Typed.c.tt_desc; at_ty = t.Typed.c.tt_ty }
...
and of_tt_desc (buffer : sbuffer) = function
  | TTconst c -> ATconst c
  | TTvar s -> ATvar s
  | TTapp (s, tts) -> ATapp (s, List.map (of_tterm buffer) tts)
  | TTinfix (t1, s, t2) -> ATinfix (of_tterm buffer t1, s, of_tterm buffer t2)
...
let rec to_tterm id { at_desc; at_ty } =
  {
...
and to_tt_desc = function
  ...
  | ATinInterval (e, lb, ub) -> TTinInterval (to_tterm 0 e, lb, ub)
...
and void_to_tform af id =
  let c =
    ...
    | AFlet (vs, l, aaf) -> TFlet (vs, to_mixed_expr l, to_tform aaf)
...
and add_aterm_list_at errors indent (buffer : sbuffer) tags iter sep = function
  | [] -> ()
...
and add_aaterm_list_at errors (indent : int) (buffer : sbuffer) tags
    ?(multi_line = false) ?(offset = 0) iter sep = function
  | [] -> ()
...
and add_aaterm_list errors (indent : int) (buffer : sbuffer) tags
  ...
  add_aaterm_list_at errors indent buffer tags ~multi_line ~offset
    buffer#end_iter sep atl
...
and add_quant_form errors (buffer : sbuffer) indent offset tags qf =
  ...
  let { aqf_bvars = bv; aqf_triggers = trs; aqf_form = aaf; aqf_hyp; _ } =
    qf.c
  in
...
and add_aform errors (buffer : sbuffer) indent tags ?parent_op aform =
  match aform with
...
and add_aaform_list_aux errors (buffer : sbuffer) indent tags ?parent_op op =
  function
  | [] -> ()
  | [ af ] ->
    (* append_buf buffer ~tags "("; *)
...
let add_to_buffer errors (buffer : sbuffer) annoted_ast =
  List.iter
    (fun (t, _) -> add_atyped_decl errors buffer t)
...
and findtags_triggers sl trs acc =
  List.fold_left (fun acc (aatl, _) -> findtags_aaterm_list sl aatl acc) acc trs
...
and findtags_aaform_list sl aafl acc =
  List.fold_left (fun acc aaf -> findtags_aaform sl aaf acc) acc aafl
...
let findtags sl l =
  List.fold_left (fun acc (td, _) -> findtags_atyped_delc sl td acc) [] l
...
let rec find_line_id_atyped_decl id td =
  if td.id < id then ()
    ...
    match td.c with
    | ATheory (_, _, _, l) -> List.iter (find_line_id_atyped_decl id) l
...
let findbyid_aaterm id aat = if aat.id = id then raise (Foundannot (AT aat))
