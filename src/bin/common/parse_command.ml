...
let model_parser = function
  ...
  | "all" -> Ok MAll
  | s -> Error (`Msg ("Option --model does not accept the argument \"" ^ s))
...
let format_parser s =
  try Ok (List.assoc s formats_list)
  with Not_found ->
    Error
...
let rule_parser = function
  ...
  | "arith" -> Ok RArith
  | "none" -> Ok RNone
  | s -> Error (`Msg ("Option --rule does not accept the argument \"" ^ s))
...
let mk_dbg_opt_spl1 debug debug_ac debug_adt debug_arith debug_arrays debug_bitv
...
let mk_case_split_opt case_split_policy enable_adts_cs max_split =
  let res =
    ...
    | "after-theory-assume" -> Ok Util.Util.AfterTheoryAssume
...
let mk_execution_opt frontend input_format parse_only parsers preludes
    no_locs_in_answers colors_in_output no_headers_in_output
    no_formatting_in_output no_forced_flush_in_output pretty_output type_only
    type_smt2 =
  ...
  let input_format =
    match input_format with None -> Native | Some fmt -> fmt
  in
...
let mk_limit_opt age_bound fm_cross_limit timelimit_interpretation steps_bound
  ...
  if steps_bound < -1 then
    ...
    let fm_cross_limit = Util.Numbers.Q.from_string fm_cross_limit in
...
let mk_sat_opt get_bottom_classes disable_flat_formulas_simplification
    ...
    no_tableaux_cdcl_in_instantiation no_tableaux_cdcl_in_theories sat_plugin
    sat_solver =
  let arith_matching = not no_arith_matching in
  let mk_no_decisions_on ndo =
    List.fold_left
      (fun set s -> match s with "" -> set | s -> Util.Util.SS.add s set)
      Util.Util.SS.empty
...
let mk_theory_opt disable_adts inequalities_plugin no_ac no_contracongru no_fm
    no_nla no_tcp no_theory restricted tighten_vars use_fpa =
  set_no_ac no_ac;
  set_no_fm no_fm;
  set_no_nla no_nla;
  set_no_tcp no_tcp;
...
let halt_opt version_info where =
  let handle_where w =
    let res =
      match w with
      | "lib" -> `Ok Util.Config.libdir
      | "plugins" -> `Ok Util.Config.pluginsdir
      | "preludes" -> `Ok Util.Config.preludesdir
    ...
    in
    match res with
    | `Ok path -> Util.Printer.print_std "%s@." path
    | `Error m -> raise (Error (false, m))
  in
  let handle_version_info vi =
    if vi then
      Util.Printer.print_std
        "@[<v 0>Version          = %s@,\
         Release date     = %s@,\
         Release commit   = %s@]@." Util.Version._version
        Util.Version._release_date Util.Version._release_commit
  in
  try
    match where with
    | Some w ->
...
let parse_dbg_opt_spl2 =
  ...
  let debug_matching =
    ...
    Arg.(
      value
      & opt int (get_debug_matching ())
      & info [ "dmatching" ] ~docv ~docs ~doc)
  in

  let debug_sat =
    let doc = "Set the debugging flag of sat." in
    Arg.(value & flag & info [ "dsat" ] ~docs ~doc)
  in
...
let parse_dbg_opt_spl3 =
  ...
  let debug_split =
    ...
    Arg.(value & flag & info [ "dsplit" ] ~docs ~doc)
  in

  let debug_sum =
    let doc = "Set the debugging flag of Sum." in
    Arg.(value & flag & info [ "dsum" ] ~docs ~doc)
  in

  let debug_triggers =
    let doc = "Set the debugging flag of triggers." in
    Arg.(value & flag & info [ "dtriggers" ] ~docs ~doc)
  in

  let debug_types =
    let doc = "Set the debugging flag of types." in
    Arg.(value & flag & info [ "dtypes" ] ~docs ~doc)
  in

  let debug_typing =
    let doc = "Set the debugging flag of typing." in
    Arg.(value & flag & info [ "dtyping" ] ~docs ~doc)
  in

  let debug_uf =
    let doc = "Set the debugging flag of uf." in
    Arg.(value & flag & info [ "duf" ] ~docs ~doc)
  in

  let debug_unsat_core =
    let doc =
      "Replay unsat-cores produced by $(b,--unsat-core). The option implies \
       $(b,--unsat-core)."
    in
    Arg.(value & flag & info [ "debug-unsat-core" ] ~docs ~doc)
  in

  let debug_use =
    let doc = "Set the debugging flag of use." in
    Arg.(value & flag & info [ "duse" ] ~docs ~doc)
  in

  let debug_warnings =
    let doc = "Set the debugging flag of warnings." in
    Arg.(value & flag & info [ "dwarnings" ] ~docs ~doc)
  in

  let rule =
    let doc =
      "$(docv) = parsing|typing|sat|cc|arith, output rule used on stderr."
    in
    let docv = "TR" in
    Arg.(value & opt rule_conv RNone & info [ "rule" ] ~docv ~docs ~doc)
  in
...
let parse_case_split_opt =
  ...
  let max_split =
    let dv = Util.Numbers.Q.to_string (get_max_split ()) in
...
let parse_execution_opt =
  ...
  let frontend =
    ...
    Arg.(
      value
      & opt string (get_frontend ())
      & info [ "frontend" ] ~docv ~docs ~doc)
  in
  ...
  let parsers =
    ...
    Arg.(
      value & opt_all string (get_parsers ()) & info [ "add-parser" ] ~docs ~doc)
  in
  ...
  let preludes =
    ...
    in
    Arg.(
      value & opt_all string (get_preludes ()) & info [ "prelude" ] ~docs ~doc)
  in
  ...
  let where =
    let doc =
      Format.sprintf
        ...
        (Arg.doc_alts [ "lib"; "plugins"; "preludes"; "data"; "man" ])
    in
    let docv = "DIR" in
...
let parse_limit_opt =
  ...
  let fm_cross_limit =
    ...
    let dv = Util.Numbers.Q.to_string (get_fm_cross_limit ()) in
    let doc =
      Format.sprintf
...
let parse_output_opt =
  ...
  let interpretation =
    let doc =
      "Experimental support for counter-example generation. Possible values \
       ...
       before instantiation (1), or before every decision (2) or instantiation \
       (3). A negative value (-1, -2, or -3) will disable interpretation \
       display. Note that $(b, --max-split) limitation will be ignored in \
       model generation phase."
    in
    let docv = "VAL" in
    Arg.(
      value
      & opt int (get_interpretation ())
      & info [ "interpretation" ] ~docv ~docs ~doc)
  in

  let model =
    let doc =
      Format.sprintf
        "Experimental support for models on labeled terms. $(docv) must be %s. \
         %s shows a complete model and %s shows all models."
        (Arg.doc_alts [ "none"; "default"; "complete"; "all" ])
        (Arg.doc_quote "complete") (Arg.doc_quote "all")
    in
    let docv = "VAL" in
    Arg.(value & opt model_conv MNone & info [ "m"; "model" ] ~docv ~doc)
  in

  let unsat_core =
    let doc = "Experimental support for computing and printing unsat-cores." in
    Arg.(value & flag & info [ "u"; "unsat-core" ] ~doc)
  in
  ...
  let output_format =
    let doc =
      Format.sprintf
        "Control the output format of the solver, $(docv) must be %s.The \
         alt-ergo native format outputs Valid/Invalid/I don't know.The smtlib \
    ...
    let docv = "FMT" in
    Arg.(
      value & opt (some format_conv) None & info [ "o"; "output" ] ~docv ~doc)
  in

  Term.(
    ret
      (const mk_output_opt $ interpretation $ model $ unsat_core $ output_format))
...
let parse_sat_opt =
  ...
  let no_tableaux_cdcl_in_instantiation =
    let doc =
      "When satML is used, this disables the use of a tableaux-likemethod for \
       instantiations with the CDCL solver."
    in
    Arg.(value & flag & info [ "no-tableaux-cdcl-in-instantiation" ] ~docs ~doc)
  in

  let no_tableaux_cdcl_in_theories =
    let doc =
      "When satML is used, this disables the use of a tableaux-likemethod for \
       theories with the CDCL solver."
    in
    Arg.(value & flag & info [ "no-tableaux-cdcl-in-theories" ] ~docs ~doc)
  in
  ...
  let sat_plugin =
    ...
    in
    Arg.(
      value & opt string (get_sat_plugin ()) & info [ "sat-plugin" ] ~docs ~doc)
  in
  ...
  let sat_solver =
    ...
    let docv = "SAT" in
    Arg.(
      value & opt string "CDCL-Tableaux"
      & info [ "sat-solver" ] ~docv ~docs ~doc)
  in
...
let parse_term_opt =
  ...
  let term_like_pp =
    ...
    Arg.(value & flag & info [ "term-like-pp" ] ~docs ~doc)
  in

  Term.(
    ret
      (const mk_term_opt $ disable_ites $ inline_lets $ rewriting $ term_like_pp))
...
let parse_theory_opt =
  ...
  let inequalities_plugin =
    ...
    Arg.(
      value
      & opt string (get_inequalities_plugin ())
  ...
  let use_fpa =
    ...
    Arg.(value & flag & info [ "use-fpa" ] ~docs ~doc)
  in
...
let parse_fmt_opt =
  ...
  let std_formatter =
    ...
    in
    Arg.(
      value & opt formatter_conv Stdout & info [ "std-formatter" ] ~docs ~doc)
  in
  ...
  let err_formatter =
    ...
    in
    Arg.(
      value & opt formatter_conv Stderr & info [ "err-formatter" ] ~docs ~doc)
  in

  Term.(ret (const mk_fmt_opt $ std_formatter $ err_formatter))

let main =
  let file =
    let doc =
      "Source file. Must be suffixed by $(i,.ae), ($(i,.mlw) and $(i,.why) are \
       depreciated, $(i,.smt2) or $(i,.psmt2)."
    in
    let i = Arg.(info [] ~docv:"FILE" ~doc) in
    Arg.(value & pos ~rev:true 0 (some string) None & i)
  in
